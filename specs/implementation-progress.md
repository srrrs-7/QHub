...
│   │   ├── middleware/          # Bearer Auth, API Key Auth, Logger
...
│   └── services/
│       ├── diffservice/         # Semantic + Text Diff
│       ├── lintservice/         # Prompt Linting
...
| 項目 | Phase | 優先度 | 備考 |
|------|-------|--------|------|
| SSE ストリーミング | 6 | 中 | コンサルチャットのリアルタイム応答 |
| RAG パイプライン | 6 | 中 | LLM オーケストレーション |
| Intent Classifier | 6 | 低 | チャット意図分類 |
| Python SDK | 3 | 中 | Go SDK と同等の機能 |
| TypeScript SDK | 3 | 中 | Go SDK と同等の機能 |
| RBAC ミドルウェア | 1 | 中 | ロールベースアクセス制御 |
| レート制限 | 1 | 低 | API レート制限 |
| Cognito/JWT 認証 | 1 | 高 | 本番認証（現在は開発用 Bearer） |
| CORS ミドルウェア | 1 | 中 | クロスオリジン対応 |
| pgvector 移行 | Emb | 低 | HNSW インデックスによる高速検索 |
| バッチ集計ジョブ | 7 | 低 | 月次ベンチマーク集計 |
| 完全な Web UI | 7 | 中 | 残りの管理画面 |
