...
| Citations 抽出 | 6 | ✅ 完了 | `ragservice/citation.go` コンテキストアイテムマッチング |
| ActionsTaken | 6 | ✅ 完了 | `actionservice/` チャットからバージョン自動作成 |
| Lint 拡張ルール | 4 | ✅ 完了 | missing-constraints + prompt-injection-risk (EN/JP) |
| Industry シードデータ | 6 | ✅ 完了 | 5業界 (healthcare, customer-support, finance, legal, education) |
| 統計的有意性テスト | 5 | ✅ 完了 | `statsservice/` Welch's t-test, Compare endpoint |
| カスタム Lint ルール | 4 | ✅ 完了 | regex ベース、反転パターン対応 |
| バッチ集計ジョブ | 7 | ✅ 完了 | `batchservice/` 月次メトリクス集計、Admin API |
| 複合認証 MW | 1 | ✅ 完了 | Bearer + API Key フォールバック認証 |
| PUT /evaluations/{id} | 3 | ✅ 完了 | 評価スコア更新 |
| PUT /consulting/sessions/{id} | 6 | ✅ 完了 | セッションクローズ |
| GET /prompts/{id}/logs | 3 | ✅ 完了 | プロンプト別ログ一覧 |
...
| org_slug 統一 | 1 | 低 | 一部エンドポイントが org_id を使用（spec は org_slug） |
